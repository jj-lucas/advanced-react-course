...
const Mutations = {
	...
	async addToCart(parent, args, ctx, info) {
		...
		// 3. Check if the item is already in their cart, and if so increment it
		if (existingCartItem) {
	...
	async removeFromCart(parent, args, ctx, info) {
		...
		if (!cartItem) throw new Error('No item found for this ID')
		// 2. Make sure they own that cartItem
	...
	async createOrder(parent, args, ctx, info) {
		// 1. Query the current user and make sure theyy are signed in
		const { userId } = ctx.request
		if (!userId) {
